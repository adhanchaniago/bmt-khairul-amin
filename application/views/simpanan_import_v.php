...
if(isset($header)) {
	...
	echo '
		...
		';
	//$hidden = $values;
	echo form_open('simpanan/import_db', '', array('val_arr' => $hidden_arr));
	echo '<input name="submit" type="submit" class="btn btn-primary" value="IMPORT DATA SIMPANAN KE DATABASE" />';
	//echo '<a href="'.site_url('simpanan/import_db').'" class="btn btn-primary">IMPORT DATA SIMPANAN KE DATABASE</a>';
	echo ' <a href="'.site_url('simpanan/import_batal').'" class="btn btn-warning">BATAL IMPORT</a>';
	echo form_close();
	$form_view = false;
}
?>
