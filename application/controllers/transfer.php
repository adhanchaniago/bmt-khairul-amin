...
class Transfer extends OperatorController {
	...
	function ajax_list() {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['detail'] ='<a href="'.site_url('cetak_transfer').'/cetak/' . $r->id . '"  title="Cetak Bukti Transaksi" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a></p>';
			$i++;
		}
		//keys total & rows wajib bagi jEasyUI
		$result = array('total'=>$data['count'],'rows'=>$rows);
